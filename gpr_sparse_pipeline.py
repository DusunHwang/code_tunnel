...
def cross_validate_model(
    ...
    **train_kwargs,
    ...
    errors_train, sigmas_train, errors_test, sigmas_test : ndarray
        Arrays of squared errors and predictive variances for each sample.
    preds_train, y_train_all, preds_test, y_test_all : ndarray
        Predictions and ground truth values aggregated over folds.
    """
    ...
    preds_train = np.concatenate([r[5] for r in results])
    y_train_all = np.concatenate([r[6] for r in results])
    preds_test = np.concatenate([r[7] for r in results])
    y_test_all = np.concatenate([r[8] for r in results])
...
def plot_prediction_comparison(pred_train, y_train, pred_test, y_test, name):
    """Scatter plot of predictions vs true values with metrics in the title."""
    r2 = r2_score(y_test, pred_test)
    mae = mean_absolute_error(y_test, pred_test)
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.scatter(y_train, pred_train, color="blue", alpha=0.5, s=10, label="Train")
    ax.scatter(y_test, pred_test, color="red", alpha=0.5, s=10, label="Test")
    min_val = min(y_train.min(), y_test.min(), pred_train.min(), pred_test.min())
    max_val = max(y_train.max(), y_test.max(), pred_train.max(), pred_test.max())
    ax.plot([min_val, max_val], [min_val, max_val], "k--", linewidth=1)
    ax.set_xlabel("True value")
    ax.set_ylabel("Predicted value")
    ax.set_title(f"{name} R2={r2:.3f} MAE={mae:.3f}")
    ax.legend()
    fig.tight_layout()
    fname = f"{name.replace(' ', '_').lower()}_pred_comparison.png"
    plt.savefig(fname)
    plt.close(fig)
...
def main():
    ...
    def run(name, fn, **kw):
        ...
        plot_prediction_comparison(pred_tr, y_tr, pred_te, y_te, name)
